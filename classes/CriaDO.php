...
include_once __DIR__ . '/ConexaoParametros.php';
...
class CriaDO {
    ...
    private function criaDTO() {
        ...
        fwrite($fp, "<?php\r\n");
        fwrite($fp, "\r\n");
        fwrite($fp, "namespace Classes\\ORM;\r\n");
        fwrite($fp, "\r\n");
        fwrite($fp, "/**\r\n");
    ...
    private function daoInserir($fp) {
        ...
        fwrite($fp, "        \$sql = new \\Classes\\SqlInserir();\r\n");
    ...
    private function daoEditarId($fp) {
        ...
        fwrite($fp, "            \$sql = new \\Classes\\SqlEditar();\r\n");
        fwrite($fp, "            // Seta a tabela\r\n");
        fwrite($fp, "            \$sql->setTabela('{$this->modulo}.{$this->tabela}');\r\n");
        ...
        fwrite($fp, "                \$criterio = new \\Classes\\SqlCriterio();\r\n");
        fwrite($fp, "                // Define criterio de edição\r\n");
        $nomeMetodo = $this->getNomeMetodo("id_{$this->tabela}");
        fwrite($fp, "                \$criterio->add(new \\Classes\\SqlFiltro('id_{$this->tabela}', '=', \${$this->tabelaPropriedade}DTO->get{$nomeMetodo}()));\r\n");
    ...
    private function daoEditar($fp) {
        ...
        foreach ($this->colunas as $linha) {
            if (isset($linha['primary_key'])) {
                $nomeMetodo = $this->getNomeMetodo($linha['nome_coluna']);
                fwrite($fp, "                \$criterio->add(new \\Classes\\SqlFiltro('{$linha['nome_coluna']}', '=', \${$this->tabelaPropriedade}DTO->get{$nomeMetodo}()));\r\n");
            }
        }
        fwrite($fp, "                \$sql->setCriterio(\$criterio);\r\n");
        fwrite($fp, "                // Prepara sql\r\n");
    ...
    private function daoExcluirId($fp) {
        ...
        fwrite($fp, "        \$sql = new \\Classes\\SqlExcluir();\r\n");
        fwrite($fp, "        // Seta a tabela\r\n");
        fwrite($fp, "        \$sql->setTabela('{$this->modulo}.{$this->tabela}');\r\n");
        fwrite($fp, "        // Instancia classe SqlCriterio\r\n");
        fwrite($fp, "        \$criterio = new \\Classes\\SqlCriterio();\r\n");
    ...
    private function daoListar($fp) {
        ...
        fwrite($fp, "        \$sql = new \\Classes\\SqlSelect();\r\n");
        fwrite($fp, "        // Seta a tabela\r\n");
        fwrite($fp, "        \$sql->setTabela('{$this->modulo}.{$this->tabela}');\r\n");
        ...
        fwrite($fp, "                \$criterio->add(new \\Classes\\SqlFiltro(\$valor[0], \$valor[1], \$valor[2]), \$operador);\r\n");
        fwrite($fp, "            }\r\n");
        fwrite($fp, "        }\r\n");
